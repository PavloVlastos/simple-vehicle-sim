...
int interface_vis_receive_byte(int sock, uint8_t verbose, int timeout_sec,
  ...
  if (status < 0) {
    if (verbose == 1) {
      printf(" |__ Interface_visualize: Failed to set socket timeout\r\n");
    }
    return ERROR;
  }
  ...
  if (bytes_received <= 0) {
    if (verbose == 1) {
      if (bytes_received == 0) {
        printf(" |__ Interface_visualize: Connection closed by peer\r\n");
      } else if (errno == EWOULDBLOCK || errno == EAGAIN) {
        printf(" |__ Interface_visualize: Receive timeout\r\n");
      } else {
        printf(" |__ Interface_visualize: Receive failed\r\n");
      }
    }
    return ERROR;
  }
...
int interface_vis_receive_float(int sock, uint8_t verbose, int timeout_sec,
  ...
  if (status != 0) {
    if (verbose) {
      printf(" |__ Interface_visualize: Failed to set socket timeout, status: "
             "%d\r\n",
             status);
    }
    return ERROR;
  ...
  if (bytes_received <= 0) {
    if (verbose) {
      if (bytes_received == 0) {
        ...
        printf(
            " |__ Interface_visualize: Receive timeout, bytes_received=%d\r\n",
            bytes_received);
      } else {
        printf(
            " |__ Interface_visualize: Receive failed, bytes_received=%d\r\n",
            bytes_received);
      }
      printf(" |__ Interface_visualize: recv error code (errno): %d\n", errno);
      printf(" |__ Interface_visualize: recv error message: %s\n",
             strerror(errno));
    }
    return ERROR;
  }
  ...
  if (bytes_received != 4) {
    if (verbose) {
      printf(" |__ Interface_visualize: Incomplete float received\r\n");
    }
    return ERROR;
  }
  ...
  if (verbose) {
    printf(" |__ Interface_visualize: Received float: %f\r\n", *data);
  }

  return SUCCESS;
}

void interface_vis_close_tcp_connection(int sock) { close(sock); }